...
Texture::Texture(const char* image, GLenum texType, GLenum slot, GLenum format, GLenum pixelType)
{
	...
	if (!bytes) {
		std::cerr << "Failed to load image using stbi_load in Texture constructor" << std::endl;
		return;
	}
	...
	GLCall(glTexImage2D(texType, 0, GL_RGB, widthImg, heightImg, 0, format, pixelType, bytes));
	// Generates MipMaps
